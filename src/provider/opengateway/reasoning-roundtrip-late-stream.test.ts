...
function toolCallLateReasoningDetailsStreamResponse(): Response {
  const events = [
    {
...
describe("OpenGateway late streamed reasoning round-trip", () => {
  it("preserves reasoning_details that arrive after a streamed tool call", async () => {
    const { bodies, fetch } = captureBodies([
      toolCallLateReasoningDetailsStreamResponse(),
